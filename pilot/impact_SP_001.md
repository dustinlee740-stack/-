...
- 분석 시각: 2026-05-08T09:00:00 (7회차 확장 — E-01~E-07 매핑 반영 + E-09 누락 행 보완) — 이전 이력: 2026-05-06 1차 → 5월 6일 동일자에 도메인 전문가 정정 5회(2차 모형 갱신=정책 본체 KOD / 3차 AML=A-Safe 외부 본체 / 4차 FDS·KFDS 역할 분담 / 5차 사유 정밀화 F-XX 매핑 / 6차 F-07 책임 분리 KOD_ITN↔TBOS) → 2026-05-07 6회차 재검증(변동 없음) → 2026-05-08 7회차 확장(E-01~E-07 본문 동기화 + E-09 행 추가)
- 분석 주체: Claude Code (재검증) → 사용자 도메인 전문가 (이전 5회 정정 결과 보존)
- 도출된 분류: 8개 (영향 들어간 분류 기준)
- 영향 가능 컴포넌트: **19개** (1차 영향 3개 / 2차 영향 16개)
- flat 인덱스 행: **58행** (F-XX 15 / E-XX 21 / S-XX 8 / 9-X 6 / 공통 8)
...
| E-09 | 고령자 고액 충전 | PCSI | Prepaid Card Service Inquiry | 65세 이상 고액 충전 시 재확인 단계 — 경계 조건 로직 검토 필요 | [components.md](components.md#pcsi) |
| E-09 | 고령자 고액 충전 | DDA | Display Data Api | 재확인 안내 문구 + 취소 버튼 동등 크기 표시 데이터 | [components.md](components.md#dda) |
| E-09 | 고령자 고액 충전 | DDV | Display Data Webview | 재확인 단계 웹뷰 노출 + 취소 버튼 동등 크기 | [components.md](components.md#ddv) |
...
**변경 사항: 본문 사유 확장 + E-09 누락 행 보완.** flat 인덱스에 E-01~E-07 추가 매핑(검증·로직·탐지 작업) 반영하고, 누락되어 있던 E-09(고령자 고액 충전) 행을 PCSI/DDA/DDV로 추가. **영향 컴포넌트 19개·위계는 변동 없음** — 동일 컴포넌트의 작업 범위(E-XX 사유)만 확장.
...
- **E-09** (고령자 고액 충전) → PCSI + DDA + DDV — 65세 이상 고액 충전 시 재확인 단계 + 취소 버튼 동등 크기 표시 경계 조건 로직 검토(이전 누락, 7회차에 보완).

flat 인덱스 카운터: 40행 → 58행 (E-XX 3 → 21).
